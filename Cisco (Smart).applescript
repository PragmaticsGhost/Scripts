tell application "Terminal"
	...
	set question to display dialog "PTY/TTY Devices will usually only allow one active connection. If there are detached screen sessions that are still using the target PTY/TTY, it will fail and display a BUSY or COULD NOT FIND PTY error. Would you like to attempt to detect and kill any previously running detached screen sessions?" buttons {"No", "Yes"} default button 2	
